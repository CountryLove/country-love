...
schema.statics.getCountrysTags = function(inputCountry) {
    ...
    const matcher = {country: inputCountry};
    
    if(inputCountry) aggregator[aggregator.length - 1].$match = matcher;
    return this.aggregate(aggregator);

};


schema.statics.getCountriesByTag = function(inputTag) {
    ...
    const matcher = {tag: inputTag};
    
    if(inputTag) aggregator[aggregator.length - 1].$match = matcher;
    return this.aggregate(aggregator);
