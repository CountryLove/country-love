...
module.exports = () => {
    MongoClient.connect(testUri)
        .then(db => {
            const rawDataColl = db.collection('countriesRaw');
            return rawDataColl.find({}).toArray().then(rawData => [rawData, db]);
        })
        .then(([rawData, db]) => {
                
            const saveCountries = rawData.map(raw => {
            ...
            return Promise.all(saveCountries).then(() => db.close());
        })
        .catch(err => {
